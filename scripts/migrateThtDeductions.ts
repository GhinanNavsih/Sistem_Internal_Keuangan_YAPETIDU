...
const MANUAL_OVERRIDES: Record<string, string> = {
  ...
  'Dina Eka Shofiana': 'Dina Eka Shofiana, SE, M.A',
  'Dina Eka Shofiana ': 'Dina Eka Shofiana, SE, M.A',
...
async function migrateTht() {
  ...
  console.log(`  - Name Column: index 25 ("NAMA")`);
  console.log(`  - THT Column: index 26 ("POTONGAN")`);
  ...
  // THT Name at index 24, Amount at index 25
  for (let i = 5; i < data.length; i++) {
    const row = data[i];
    const rawName = String(row[24] || '').trim();
    if (!rawName) continue;

    const rawAmount = row[25];
    let amount = 0;
  ...
  // Create complete updates list (all active employees must have tht.deductionAmount set, to either amount or 0)
  ...
  console.log('═══════════════════════════════════════════════════════════');
  console.log(`  Total Rows with THT in Excel: ${matched.length + unmatched.length}`);
  console.log(`  Matched Active Employees: ${matched.length}`);
  console.log(`  Unmatched Excel Rows: ${unmatched.length}`);
  console.log(`  Active Employees with Rp 0 THT: ${employees.length - matched.length}`);
  ...
  if (!isCommit) {
    ...
    console.log('   npx tsx scripts/migrateThtDeductions.ts --commit\n');
    return;
  }
  ...
  console.log(`\n🎉 Success! Migrated Potongan Tabungan Hari Tua BNI Simponi deductions for all active Loyalis employees.`);
}

migrateTht().catch(console.error);

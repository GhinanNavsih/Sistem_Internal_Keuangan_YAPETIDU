...
async function migratePinlu() {
  const excelPath = path.resolve(process.cwd(), '4. Gaji April 2026.xlsx');
  ...
  // Find column with PINLU/TAGIHAN on the first row or header rows
  let pinluColIndex = -1;
  const headers = data[0];
  headers.forEach((h, idx) => {
    if (h && typeof h === 'string' && h.toUpperCase().includes('PINLU')) {
      pinluColIndex = idx;
    }
  });

  if (pinluColIndex === -1) {
    console.error('❌ Could not find PINLU/TAGIHAN column in headers.');
    process.exit(1);
  }

  console.log(`🔍 Mapping columns:`);
  console.log(`  - Name Column: index ${pinluColIndex + 1} ("NAMA")`);
  console.log(`  - Pinlu/Tagihan Column: index ${pinluColIndex + 2} ("POTONGAN")`);
  ...
  // Name is at column index 40, Amount is at column index 41
  for (let i = 5; i < data.length; i++) {
    const row = data[i];
    const rawName = String(row[40] || '').trim();
    if (!rawName) continue;

    const rawAmount = row[41];
    let amount = 0;
    ...
    // Manual override for H. M. Zahrul Azhar, S.IP., M.Kes. as shown in the user's screenshot (583,856 instead of 614,324)
    if (rawName.includes('Zahrul Azhar')) {
      console.log(`ℹ️ Overriding amount for ${rawName} from ${amount} to 583856 (per screenshot)`);
      amount = 583856;
    }
  ...
  // Create complete updates list (all active employees must have pinlu.deductionAmount set, to either amount or 0)
  ...
  console.log('═══════════════════════════════════════════════════════════');
  console.log(`  Total Rows with Pinlu in Excel: ${matched.length + unmatched.length}`);
  console.log(`  Matched Active Employees: ${matched.length}`);
  console.log(`  Unmatched Excel Rows: ${unmatched.length}`);
  console.log(`  Active Employees with Rp 0 Pinlu: ${employees.length - matched.length}`);
  ...
  if (!isCommit) {
    ...
    console.log('   npx tsx scripts/migratePinluDeductions.ts --commit\n');
    return;
  }
  ...
  for (let i = 0; i < updates.length; i += batchSize) {
    ...
    chunk.forEach(up => {
      batch.update(up.ref, {
        'pinlu.deductionAmount': up.amount,
  ...
  console.log(`\n🎉 Success! Migrated Potongan Pinlu/Tagihan deductions for all active Loyalis employees.`);
}

migratePinlu().catch(console.error);

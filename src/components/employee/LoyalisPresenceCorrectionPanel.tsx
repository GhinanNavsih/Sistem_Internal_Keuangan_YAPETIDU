...
type LoyalisWorkflowMode = 'all' | 'presence_correction' | 'sick_leave';

export function LoyalisPresenceCorrectionPanel({
  workflowMode = 'all',
  embedded = false,
}: {
  workflowMode?: LoyalisWorkflowMode;
  embedded?: boolean;
}) {
  ...
  const isTimeRangeInvalid = useMemo(() => {
    if (effectiveType === 'izin_resmi') return false;
    if (effectiveType !== 'both' || !checkInTime || !checkOutTime) return false;
  ...
  }, [effectiveType, checkInTime, checkOutTime]);
  ...
  const visibleRequests = useMemo(() => requests.filter((request) => {
    if (workflowMode === 'sick_leave') return request.type === 'izin_resmi';
    if (workflowMode === 'presence_correction') return request.type !== 'izin_resmi';
    return true;
  }), [requests, workflowMode]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const checkIn = effectiveType === 'izin_resmi' ? '07:30' : (checkInTime.trim() || '08:00');
    const checkOut = effectiveType === 'izin_resmi' ? '14:00' : (checkOutTime.trim() || '14:00');

    if (effectiveType !== 'tap_out' && !isValidClockTime(checkIn)) {
      setMessage({ type: 'error', text: 'Masukkan jam masuk dalam format HH:MM yang valid.' });
      return;
    }
    if (effectiveType !== 'tap_in' && !isValidClockTime(checkOut)) {
      ...
      if (targetId) {
        ...
        setMessage({
          ...
          text: workflowMode === 'sick_leave'
            ...
            : 'Koreksi presensi berhasil diajukan!',
        });
      }
      ...
      setMessage({
        ...
        text: err instanceof Error
          ...
          : workflowMode === 'sick_leave'
            ? 'Gagal mengajukan izin sakit. Silakan coba lagi.'
            : 'Gagal mengajukan koreksi presensi. Silakan coba lagi.',
      });
    } finally {
      setSubmitLoading(false);
    }
  ...
  return (
      <div className={embedded ? 'space-y-6' : 'min-h-screen bg-slate-50/50 py-8 px-4 sm:px-6'}>
      <style>{`
      ...
      <div className={embedded ? 'space-y-6' : 'max-w-6xl mx-auto space-y-6'}>
        ...
        {!embedded && <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <h1 className="text-lg font-extrabold text-slate-950 uppercase tracking-tight">
              {workflowMode === 'sick_leave' ? 'Izin Sakit' : 'Koreksi Presensi'}
            </h1>
        ...
        <div className={embedded ? 'grid grid-cols-1 gap-6' : 'grid grid-cols-1 lg:grid-cols-12 gap-6'}>
            ...
            <Card className="bg-white rounded-3xl border-none shadow-[0_4px_25px_rgba(0,0,0,0.02)] p-6 space-y-6">
              <div>
                <CardTitle className="text-base font-extrabold text-slate-850 tracking-wide uppercase flex items-center gap-2">
                  ...
                  {editingRequestId
                    ? workflowMode === 'sick_leave' ? 'Ubah Izin Sakit' : 'Ubah Koreksi'
                    : workflowMode === 'sick_leave' ? 'Ajukan Izin Sakit' : 'Ajukan Koreksi'}
                </CardTitle>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="space-y-1.5">
                  <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block">
                    {workflowMode === 'sick_leave' ? 'Tanggal Sakit' : 'Tanggal Presensi'}
                  </label>
                  <Input
                    type="date"
                    value={date}
                  ...
                  >
                    <SelectTrigger className="w-full rounded-xl border-slate-200 bg-white h-11 text-sm font-semibold focus:ring-2 focus:ring-indigo-500/20">
                      <SelectValue>
                        ...
                        {effectiveType === 'tap_out' && 'Hanya Scan Pulang'}
                    ...
                    <SelectContent className="bg-white rounded-xl border border-slate-100 shadow-xl">
                      ...
                      {workflowMode === 'all' && <SelectItem value="izin_resmi">Izin Resmi (Hari Penuh)</SelectItem>}
                    </SelectContent>
                  </Select>
                </div>}
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  {effectiveType !== 'tap_in' && effectiveType !== 'izin_resmi' && (
                    <div className="space-y-1.5">
                ...
                <div className="space-y-1.5">
                  <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block">
                    {workflowMode === 'sick_leave' ? 'Keterangan Sakit' : 'Alasan / Keterangan'}
                  </label>
                  <textarea
                    rows={3}
                    value={reason}
            ...
            <Card className="bg-white rounded-3xl border-none shadow-[0_4px_25px_rgba(0,0,0,0.02)] p-6">
              <div className="mb-4">
                <CardTitle className="text-base font-extrabold text-slate-850 tracking-wide uppercase flex items-center gap-2">
                  ...
                  {workflowMode === 'sick_leave' ? 'Riwayat Izin Sakit' : 'Riwayat Koreksi Presensi'}
                </CardTitle>
                <CardDescription className="text-xs text-slate-450 mt-1">
                  {workflowMode === 'sick_leave'
                    ? 'Daftar izin sakit yang Anda ajukan.'

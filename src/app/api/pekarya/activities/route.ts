...
import { parsePhotoAuditMetadata, parseProofPhoto } from '@/lib/server/photoEvidence';
...
function formatJakartaClockTime(date: Date): string {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const isPhotoOnlyReport = isFoundItem || isReprimand;
    // On a self-authorized SPJ, "Otorisasi & Mulai Perjalanan" IS the departure,
    // so its server stamp is the departure time and the submitted form is never
    // trusted for it — a modified client cannot backdate a trip to earn a night
    // premium or a longer duration. This holds only for self-created journeys:
    // on a Kepala-Satker-assigned one `authorizedAt` records when the assignment
    // was raised, which can be days before the sopir actually drives, so those
    // keep the sopir's own reported departure.
    const reportedJourneyId =
      typeof command.driverData?.journeyId === 'string' ? command.driverData.journeyId : null;
    if (reportedJourneyId) {
      const authorizedSnapshot = await adminDb
        .collection('DriverJourneys')
        .doc(reportedJourneyId)
        .get();
      const authorizedJourney = authorizedSnapshot.data();
      const authorizedAt = authorizedJourney?.authorizedAt;
      const authorizedDate =
        authorizedAt && typeof authorizedAt.toDate === 'function' ? authorizedAt.toDate() : null;
      // Gated on the journey's own flag, not on it merely being self-created:
      // only journeys authorized under this rule carry it, so reports already in
      // flight when this shipped keep the departure their sopir entered.
      if (authorizedDate && authorizedJourney?.departureLockedToAuthorization === true) {
        command.timeStart = formatJakartaClockTime(authorizedDate);
        command.activityDate = formatJakartaDate(authorizedDate);
        command.driverData!.dateStart = command.activityDate;
      }
    }
    ...
    const result = await adminDb.runTransaction(async (transaction) => {
      ...
      if (journeyId) {
        ...
        // A self-authorized journey has no route — and so no fuel figure — until
        // this report supplies one, so its BBM is reserved here rather than at
        // authorization. Already-held reservations keep their original amount;
        // the auditor reconciles any difference when approving.
        ...
        const baseOperationalCost = !reservationAlreadyHeld
          // ActivityReports store the submitted round-trip distance, which the
          // vehicle rate already prices per kilometre travelled.
          ? Math.ceil(Number(driverData.distanceKm || 0) * getDriverVehicleRate(vehicleType))
        ...
        if (!reservationAlreadyHeld && fuelProcurementMode !== DEFAULT_FUEL_PROCUREMENT_MODE) {
          ...
          fuelReservationAfterSubmission = reserveFuel(fuelContext, {
            ...
            reason: 'Reservasi BBM saat pengajuan laporan perjalanan',
          });
          procuredAccumulatedAmount = fuelReservationAfterSubmission.procuredAccumulatedAmount;
        } else if (!reservationAlreadyHeld) {
          fuelReservationAfterSubmission = {
            fuelReservationVersion: CURRENT_FUEL_RESERVATION_VERSION,
            fuelReservationId:
              existingReservation?.fuelReservationId ||
              `FUEL-${journeyId}-${randomUUID().replaceAll('-', '').slice(0, 16).toUpperCase()}`,
            fuelReservationState: 'none',
            fuelReservationVehicleName: vehicleType,
            fuelProcurementMode,
            baseFuelAllowance: baseOperationalCost,
            heldFuelAmount: 0,
      ...
      const after = {
        ...
        ...(typeof journeyBefore?.authorizationProofPhotoUrl === 'string'
          ? {
              authorizationProofPhotoUrl: journeyBefore.authorizationProofPhotoUrl,
              authorizationProofPhotoAuditMetadata:
                journeyBefore.authorizationProofPhotoAuditMetadata ?? null,
            }
          : {}),
      };

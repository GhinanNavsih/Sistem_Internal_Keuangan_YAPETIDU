...
export default function UraianPage() {
  ...
  const [focusTarget, setFocusTarget] = useState<{ table: 'pemasukan' | 'pengeluaran'; row: number; col: number } | null>(null);
  ...
  useEffect(() => {
    if (focusTarget) {
      const input = document.querySelector(
        `[data-table="${focusTarget.table}"][data-row="${focusTarget.row}"][data-col="${focusTarget.col}"]`
      ) as HTMLInputElement;
      if (input) {
        input.focus();
        input.select();
  ...
  }, [focusTarget, pemasukanRows, pengeluaranRows]);

  // Keyboard navigation handler for tables
  const handleTableKeyDown = (
    ...
    const rows = table === 'pemasukan' ? pemasukanRows : pengeluaranRows;
    const maxCols = 4; // Uraian (0), Qty (1), Rate (2), Realisasi (3)
    ...
    if (e.key === 'Enter' && e.altKey) {
      ...
      // Check if the input text is fully selected/highlighted
      const isFullySelected = input.selectionStart === 0 && input.selectionEnd === input.value.length && input.value.length > 0;
      
      if (isFullySelected) {
        // Deselect and place cursor at the end of the text
        input.setSelectionRange(input.value.length, input.value.length);
      } else {
        // Normal Enter behavior: move down or add new row
        if (rowIdx === rows.length - 1) {
          if (table === 'pemasukan') {
            setPemasukanRows(prev => [...prev, { uraian: '', rincianQty: '', rincianRate: 0, realisasi: 0 }]);
            setFocusTarget({ table: 'pemasukan', row: rowIdx + 1, col: colIdx });
          } else {
            setPengeluaranRows(prev => [...prev, { type: 'item', uraian: '', rincianQty: '', rincianRate: 0, realisasi: 0 }]);
            setFocusTarget({ table: 'pengeluaran', row: rowIdx + 1, col: colIdx });
          }
        } else {
          const nextInput = document.querySelector(
            `[data-table="${table}"][data-row="${rowIdx + 1}"][data-col="${colIdx}"]`
          ) as HTMLInputElement;
          if (nextInput) {
            nextInput.focus();
            nextInput.select();
      ...
      if (colIdx < maxCols - 1) {
        const nextInput = document.querySelector(
          `[data-table="${table}"][data-row="${rowIdx}"][data-col="${colIdx + 1}"]`
        ) as HTMLInputElement;
        if (nextInput) {
          nextInput.focus();
          nextInput.select();
        ...
        if (rowIdx < rows.length - 1) {
          const nextInput = document.querySelector(
            `[data-table="${table}"][data-row="${rowIdx + 1}"][data-col="0"]`
          ) as HTMLInputElement;
          if (nextInput) {
            nextInput.focus();
            nextInput.select();
      ...
      if (colIdx > 0) {
        ...
        e.preventDefault();
        const prevInput = document.querySelector(
          `[data-table="${table}"][data-row="${rowIdx - 1}"][data-col="${maxCols - 1}"]`
        ) as HTMLInputElement;
        if (prevInput) {
          prevInput.focus();
          prevInput.select();
  ...
  useEffect(() => {
    ...
    if (!pelaporanReportName.trim() || !pelaporanDept) {
      return;
    }
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        // Construct a unified realisasiRows list for backward compatibility
  ...
  const loadPelaporanFromData = (rpt: any) => {
    ...
    if (rpt.pemasukanRows && rpt.pengeluaranRows) {
      ...
      // Migrate from old flat realisasiRows
      ...
      rpt.realisasiRows.forEach((row: any) => {
        if (row.type === 'group_header') {
          ...
          if (lowerUraian.includes('pengembangan')) {
            currentSection = 'pengembangan';
          ...
          } else if (lowerUraian.includes('pemasukan')) {
          ...
          if (currentSection === 'pemasukan') {
            parsedPemasukan.push(row);
          } else if (currentSection === 'pengembangan') {
            const lowerUraian = row.uraian.toLowerCase();
            const pct = parseFloat(row.rincianQty) || 20;
            if (lowerUraian.includes('yayasan')) {
              parsedYayasanPct = pct;
            } else if (lowerUraian.includes('unipdu')) {
              parsedUnipduPct = pct;
            }
          } else {
            parsedPengeluaran.push(row);
    ...
    if (parsedPemasukan.length === 0) {
      parsedPemasukan = [{ uraian: '', rincianQty: '', rincianRate: 0, realisasi: 0 }];
    }
    if (parsedPengeluaran.length === 0) {
      parsedPengeluaran = [{ type: 'item', uraian: '', rincianQty: '', rincianRate: 0, realisasi: 0 }];
    }
            ...
            {activeTab === 'vakasi_loyalis' && (
              <div className="flex flex-wrap gap-2">
                ...
                <Button
                  ...
                  variant="outline"
                  className="rounded-xl border-slate-200 text-slate-700 bg-slate-50 hover:bg-slate-100 hover:border-slate-300 transition-all font-semibold flex items-center gap-2 shadow-sm cursor-pointer"
                >
                ...
                <Button
                  ...
                  variant="outline"
                  className="rounded-xl border-emerald-200 text-emerald-700 bg-emerald-50 hover:bg-emerald-100 hover:border-emerald-300 transition-all font-semibold flex items-center gap-2 shadow-sm cursor-pointer"
                >
          ...
          <div className="flex flex-col gap-8">
            ...
            <Card className="bg-white rounded-[20px] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border-none overflow-visible p-4 md:p-6 space-y-5 animate-in fade-in duration-500">
              ...
              {realisasiEnabled && (() => {
                ...
                // Dana Operasional (Budget available)
                    ...
                    {expandedSections.realisasi && (
                      <div className="p-4 md:p-5 space-y-6 bg-white">
                            ...
                            <table className="w-full text-left border-collapse">
                              ...
                              <tbody>
                                {pemasukanRows.map((row, idx) => {
                                  ...
                                  return (
                                    <tr key={idx} className="border-b border-slate-50 hover:bg-slate-50/30 transition-colors">
                                      ...
                                      <td className="px-3 py-2"><Input type="text" inputMode="numeric" placeholder="0" value={row.realisasi > 0 ? fmtRp(row.realisasi) : ''} onChange={(e) => { const val = parseInt(e.target.value.replace(/\D/g, ''), 10) || 0; setPemasukanRows(prev => { const c = [...prev]; c[idx] = { ...c[idx], realisasi: val }; return c; }); }} onKeyDown={(e) => handleTableKeyDown(e, 'pemasukan', idx, 3)} data-table="pemasukan" data-row={idx} data-col={3} className="rounded-lg border-slate-200 font-bold text-slate-900 text-xs h-8 w-full text-right" /></td>
                                      <td className="px-3 py-2 text-center">
                                        <div className="flex items-center justify-center gap-1">
                                          ...
                                          }} className="h-7 w-7 text-slate-450 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg cursor-pointer">
                                            <Plus className="w-3.5 h-3.5" />
                                          </Button>
                                          <Button type="button" variant="ghost" size="icon" title="Hapus baris" onClick={() => setPemasukanRows(prev => prev.filter((_, i) => i !== idx))} className="h-7 w-7 text-rose-450 hover:text-rose-600 hover:bg-rose-50 rounded-lg cursor-pointer">
                                            <Trash2 className="w-3.5 h-3.5" />
                                          </Button>
                                        </div>
                                      </td>
                                ...
                                <tr className="bg-slate-50 border-t border-slate-200 font-semibold">
                                  ...
                                  <td className="px-3 py-2.5 text-xs font-black text-slate-800 text-right font-mono">{fmtRp(totalPemasukanAnggaran)}</td>
                                  <td className="px-3 py-2.5 text-xs font-black text-slate-800 text-right font-mono">{fmtRp(totalPemasukanRealisasi)}</td>
                                  <td></td>
                                </tr>
                              </tbody>
                            </table>
                        ...
                        <div className="bg-slate-50/55 border border-slate-150 rounded-2xl p-4 md:p-5 space-y-4">
                          <span className="text-xs font-bold text-slate-800 uppercase tracking-wider bg-white px-2.5 py-1 rounded-lg border border-slate-200 shadow-2xs block w-fit font-sans">2. Alokasi Dana Pengembangan & Operasional</span>
                                  ...
                                  if (row.type === 'group_header') {
                                    return (
                                      <tr key={idx} className="bg-slate-50/60 border-b border-slate-100">
                                        ...
                                        <td colSpan={5} className="px-3 py-2.5"><Input type="text" placeholder="Nama grup (e.g., A. Pengeluaran)..." value={row.uraian} onChange={(e) => { const val = e.target.value; setPengeluaranRows(prev => { const c = [...prev]; c[idx] = { ...c[idx], uraian: val }; return c; }); }} onKeyDown={(e) => handleTableKeyDown(e, 'pengeluaran', idx, 0)} data-table="pengeluaran" data-row={idx} data-col={0} className="rounded-lg border-slate-200 font-bold text-slate-800 text-xs h-8 w-full bg-transparent" /></td>
                                        <td className="px-3 py-2.5 text-center">
                                          <div className="flex items-center justify-center gap-1">
                                            ...
                                            }} className="h-7 w-7 text-slate-450 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg cursor-pointer">
                                              <Plus className="w-3.5 h-3.5" />
                                            </Button>
                                            <Button type="button" variant="ghost" size="icon" title="Hapus grup" onClick={() => setPengeluaranRows(prev => prev.filter((_, i) => i !== idx))} className="h-7 w-7 text-rose-450 hover:text-rose-600 hover:bg-rose-50 rounded-lg cursor-pointer">
                                              <Trash2 className="w-3.5 h-3.5" />
                                            </Button>
                                          </div>
                                        </td>
                                  ...
                                  return (
                                    <tr key={idx} className="border-b border-slate-50 hover:bg-slate-50/30 transition-colors">
                                      ...
                                      <td className="px-3 py-2 text-center">
                                        <div className="flex items-center justify-center gap-1">
                                          <div className="relative">
                                            <Button type="button" variant="ghost" size="icon" title="Sisipkan baris di bawah" onClick={() => setActiveInsertMenuIdx(activeInsertMenuIdx === idx ? null : idx)} className="h-7 w-7 text-slate-450 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg cursor-pointer">
                                              <Plus className="w-3.5 h-3.5" />
                                            </Button>
                                          ...
                                          <Button type="button" variant="ghost" size="icon" title="Hapus baris" onClick={() => setPengeluaranRows(prev => prev.filter((_, i) => i !== idx))} className="h-7 w-7 text-rose-450 hover:text-rose-600 hover:bg-rose-50 rounded-lg cursor-pointer">
                                            <Trash2 className="w-3.5 h-3.5" />
                                          </Button>
                                        </div>
                                      </td>
                            ...
                            {vakasiPengujiRows.map((row, idx) => {
                              ...
                              return (
                                <tr key={idx} className={`border-b border-slate-50 hover:bg-slate-50/30 transition-colors ${row.showDropdown ? 'relative z-50' : 'relative z-0'}`}>
                                  ...
                                  <td className="px-3 py-2 text-center"><Button type="button" variant="ghost" size="icon" onClick={() => setVakasiPengujiRows(prev => prev.filter((_, i) => i !== idx))} className="h-7 w-7 text-rose-450 hover:text-rose-600 hover:bg-rose-50 rounded-lg cursor-pointer"><Trash2 className="w-3.5 h-3.5" /></Button></td>
                                </tr>
                              );
                            })}
                            ...
                            {kepanitiaaanRows.map((row, idx) => {
                              ...
                              return (
                                <tr key={idx} className={`border-b border-slate-50 hover:bg-slate-50/30 transition-colors ${row.showDropdown ? 'relative z-50' : 'relative z-0'}`}>
                                  ...
                                  <td className="px-3 py-2 text-xs font-black text-slate-900 text-right font-mono">{fmtRp(rowTotal)}</td>
                                  <td className="px-3 py-2 text-center"><Button type="button" variant="ghost" size="icon" onClick={() => setKepanitiaaanRows(prev => prev.filter((_, i) => i !== idx))} className="h-7 w-7 text-rose-455 hover:text-rose-600 hover:bg-rose-50 rounded-lg cursor-pointer"><Trash2 className="w-3.5 h-3.5" /></Button></td>
                                </tr>
                              );
                            })}
                            {/* Action Row inside Table Body */}
                ...
                <div>
                 ...
                 <div className="flex gap-2 items-center flex-wrap">
                  {autoSaveStatus === 'saving' && (
                    <span className="text-slate-450 text-[11px] font-medium flex items-center gap-1.5 animate-pulse mr-2 font-sans">

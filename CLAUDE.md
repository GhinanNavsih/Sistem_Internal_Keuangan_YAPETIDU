...
**Attendance / presence**: `LoyalisPresence` (monthly import, keyed by period), `LoyalisPresenceCorrections`, `AttendanceImports` (blue-collar/Pekarya raw import batches), `GantiLiburRequests` + `GantiLiburRequestRevisions` (Loyalis compensatory day off, server-only; see `gantiLibur.ts` below).
...
- **`gantiLibur.ts`** — Loyalis-only ganti libur (compensatory day off): working a full 07:30–14:00 on a Jumat/Tanggal Merah earns one working day off (less is lembur, which SAKU does not pay — the request is declined); max 2 days off per Saturday–Friday week (the week ends on the Loyalis weekly day off); the worked holiday and the day off may come in either order. Employees submit from `/employee/leave`; Loyalis Admin/Super Admin approve on `/dashboard/payroll/uraian/presence-corrections` only once the worked date's `LoyalisPresence` shows the full window. An approved day off is overlaid onto presence as a `GANTI LIBUR` row by the same `applyApprovedPaidLeaveToLoyalisEntry` that writes `CUTI` rows. A request may carry up to 5 optional surat resmi files (`gantiLiburAttachments.ts`): uploaded through `/api/uploads/ganti-libur` into Storage `ganti_libur/{employeeId}/`, then named by path on submit — the server re-reads each object's metadata (`loadGantiLiburAttachments`) instead of trusting a client-sent URL.
